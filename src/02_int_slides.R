...
# French plot (can comment out)

#df_int_filter_fr <- df_int_filter %>% 
#  ungroup() %>%
...
#  
#
#ggplot(df_int_filter_fr, aes(case_100, sdma_pop_million, group = countryterritoryCode, colour = countryterritoryCode)) +
#  geom_line(size = 2) +
#  geom_text_repel(aes(label = label),
#                  size = 6,
#                  nudge_x = 6,
#                  nudge_y = 5,
#                  na.rm = TRUE) +
#  scale_y_continuous("Nombre de cas quotidien pour 1 000 000 personnes", expand = c(0, 0), limits = c(0,NA)) +
#  scale_x_continuous("Nombre de jours suivant la déclaration du 100éme cas dans le pays correspondant", expand = c(0, 0), limits = c(0,NA)) +
