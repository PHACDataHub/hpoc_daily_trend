...
df_filter <- df %>% filter(prname == jurisdiction) %>%
  filter(date >= "2020-03-08")

table_filter_case <- data.frame(desc = c(paste0("Reported on ", Sys.Date()), "7-day moving average (per day):", "Weekly percent change"),
                                value = c(df_filter %>% filter(date == max(date)) %>% select(numtoday) %>% pull(),
...
table_filter_death <- data.frame(desc = c(paste0("Reported on ", Sys.Date()), "7-day moving average (per day):", "Weekly percent change"),
                                 value = c(df_filter %>% filter(date == max(date)) %>% select(numdeathstoday) %>% pull(),
...
p3 <- ggplot(df_filter, aes(x = date, y = numtoday)) +
  ...
  scale_x_date(
    ...
    labels = label_date("%d%b"),
    limits = c(df_filter %>% filter(date == max(date)) %>% select(date) %>% pull() - weeks(2), 
